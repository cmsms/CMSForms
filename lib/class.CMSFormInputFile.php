...
class CMSFormInputFile extends CMSFormInput
{
    ...
    {
        if(isset($_FILES[$this->id . $this->name]))
        {
            return $_FILES[$this->id . $this->name];
        }
        return null;
    }
    ...
    protected function getUploadField()
    {
        ...
        if (!$this->isEmpty()) {
            if (isset($this->settings['direct_link']) && $this->settings['direct_link'] != '') {
                ...
                if ((substr($file_url, -1) != '/') && (substr($this->getValue(), 0, 1) != '/')) {
                    $file_url .= '/';
                }
            ...
            if (isset($this->settings['delete_checkbox'])) {
                $field .= ' ' . $this->getModule()->CreateInputCheckbox(
                        $this->id,
                        $this->settings['delete_checkbox'],
                        '1'
                    ) . ' ' . $this->getModule()->lang('delete');
            }
        }
